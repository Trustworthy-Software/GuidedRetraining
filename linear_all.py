...
def parse_option():
    ...
    opt.dataset = "NEW_{}_all".format(opt.approach)
    if opt.long == "yes":
...
def get_ckpt(opt):
    ...
    dataset_4p = "NEW_{}_all".format(opt.approach)
    ...
    try: #some models use batch size 20
        ...
        if opt.long == "yes":
            ckpt_ = [f 
                     for f 
                     in listdir(save_folder_4p) if 
                     (isfile(join(save_folder_4p, f)) and 
                      f.startswith("LONG"))]
        else:
            ckpt_ = [f 
                     for f 
...
def validate(val_loader, model, classifier, criterion, opt):
    ...
    with torch.no_grad():
        ...
        for idx, (images, labels) in enumerate(val_loader):
            ...
            #print(output[:4], output_argmax[:4], labels[:4])
    ...
    acc, pre, rec, f1, tn, fp, fn, tp = my_metrics.get_metrics(reduction='none')
    print("\tVa lo, ac, pr, re, f1, tn, fp, fn, tp", np.round(losses.avg, 4), 
          np.round(acc, 4), np.round(pre, 4), np.round(rec, 4), 
          np.round(f1, 4), tn, fp, fn, tp)
    return np.round(losses.avg, 4), np.round(f1, 4)
...
def main():
    ...
    (train_loader, val_loader, test_loader, 
     x_train1, x_valid1, x_test1) = set_loader(opt)

    # build model and criterion
    model, classifier, criterion = set_model(opt)
    
    train_loader = get_transformations_from_model(
        train_loader, x_train1, model, opt, shuffle=True, 
        drop_last=opt.drop_last_train)
    val_loader = get_transformations_from_model(
        val_loader, x_valid1, model, opt, shuffle=False)
    test_loader = get_transformations_from_model(
        test_loader, x_test1, model, opt, shuffle=False)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # eval for one epoch
        loss, val_f1 = validate(val_loader, 
                                model, classifier, criterion, opt)
