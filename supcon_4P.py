...
def parse_option():
    ...
    parser.add_argument(
        ...
        help='which guided retraining sub-model, either 1, 2, 3 or 4')
    ...
    parser.add_argument(
        '--es_brk', type=lambda x: (str(x).lower() == 'true'), default=True,
        help='es_brk')
    ...
    #define sub-models of guided retraining
    if int(opt.part) == 1:
        opt.first = 0
        opt.second = 2
    elif int(opt.part) == 2:
    ...
    opt.dataset = "NEW_{}_diff_4P_{}".format(opt.approach, opt.part)
...
def set_loader(opt):
    ...
    indexes_diff_tr = indexes[opt.first] + indexes[opt.second]
    ...
    if opt.fs == True:
        ...
        scores_indices = load_list(
            os.path.join(opt.path_indices, "indices_{}".format(opt.split_id), 
                         "scores_indices_mutual_info_{}_p{}".format(opt.approach, 
                                                                    opt.part)))
...
def main():
    ...
    for epoch in range(1, opt.epochs + 1):
        # train for one epoch
        time1 = time.time()
        loss = train(train_loader, x_train1, model, 
